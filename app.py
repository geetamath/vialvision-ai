...
# FIXED THRESHOLDS
# ─────────────────────────────────────────────
IMAGE_CONF  = 0.7   # High confidence for static images
VIDEO_CONF  = 0.5   # Lower for video tracking
CAMERA_CONF = 0.5   # Live camera
IOU_THRESH  = 0.3   # Low IoU to suppress overlapping boxes
...
# ─────────────────────────────────────────────
# DRAW BOXES — Image Tab
# Vial label ABOVE box, Quality label BELOW box
...
def draw_boxes(img_bgr, results, model):
    ...
    for box in results[0].boxes:
        ...
        cv2.putText(img_bgr, label, (lx + 4, ly),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
    return img_bgr

# ─────────────────────────────────────────────
# DRAW VIDEO LABELS — Video & Camera Tab
# Vial label ABOVE, Quality label BELOW
# ─────────────────────────────────────────────
...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .verdict {{ background:#0f1e35; border-left:5px solid {verdict_color}; padding:1rem 2rem; display:flex; justify-content:space-between; align-items:center; }}
  ...
  .section-title {{ font-size:0.75rem; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:2px; margin-bottom:1.2rem; padding-bottom:0.5rem; border-bottom:2px solid #e2e8f0; }}
  .stat-grid {{ display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; }}
  .stat-card {{ border-radius:10px; padding:1.2rem; text-align:center; }}
  ...
  .stat-label {{ font-size:0.7rem; font-weight:600; text-transform:uppercase; letter-spacing:1.5px; margin-top:0.4rem; color:#64748b; }}
  ...
  .bar-val {{ width:60px; text-align:right; font-family:'Share Tech Mono',monospace; font-size:0.85rem; color:#334155; }}
  ...
  .report-footer {{ background:#f8fafc; padding:1.5rem 3rem; display:flex; justify-content:space-between; align-items:center; }}
...
<div class="page">
  <div class="header">
    <div class="header-top">
      <div>
        ...
        <div style="font-size:0.8rem;color:#64a0c8;margin-top:0.3rem;letter-spacing:1px;">PHARMACEUTICAL INSPECTION SYSTEM</div>
      </div>
      <div class="report-id">REPORT ID<b>{report_id}</b>{date_str}<br>{time_str}</div>
    </div>
  ...
  <div class="verdict">
    <div><div class="verdict-label">Overall Quality Verdict</div><div class="verdict-value">{verdict}</div></div>
    <div style="text-align:center;"><div class="verdict-label">Pass Rate</div><div class="verdict-value">{pass_rate:.1f}%</div></div>
    <div style="text-align:right;"><div class="verdict-label">Defect Rate</div><div class="verdict-value" style="color:{verdict_color}">{defect_rate:.1f}%</div><div style="font-size:0.8rem;color:#64a0c8;">Threshold: &lt;5% = PASS</div></div>
  </div>
  <div class="section">
    <div class="section-title">📊 Detection Summary</div>
    <div class="stat-grid">
      <div class="stat-card total"><div class="stat-val">{total}</div><div class="stat-label">Total Vials</div></div>
      <div class="stat-card sealed"><div class="stat-val">{sealed}</div><div class="stat-label">Sealed</div></div>
      <div class="stat-card unsealed"><div class="stat-val">{unsealed}</div><div class="stat-label">Unsealed</div></div>
      <div class="stat-card damaged"><div class="stat-val">{damaged}</div><div class="stat-label">Damaged</div></div>
    </div>
    <div style="margin-top:1.5rem;">
      <div class="bar-row"><div class="bar-label">Sealed</div><div class="bar-track"><div class="bar-fill" style="width:{sealed/max(total,1)*100:.1f}%;background:#22c55e;"></div></div><div class="bar-val">{sealed/max(total,1)*100:.1f}%</div></div>
      <div class="bar-row"><div class="bar-label">Unsealed</div><div class="bar-track"><div class="bar-fill" style="width:{unsealed/max(total,1)*100:.1f}%;background:#f97316;"></div></div><div class="bar-val">{unsealed/max(total,1)*100:.1f}%</div></div>
      <div class="bar-row"><div class="bar-label">Damaged</div><div class="bar-track"><div class="bar-fill" style="width:{damaged/max(total,1)*100:.1f}%;background:#f43f5e;"></div></div><div class="bar-val">{damaged/max(total,1)*100:.1f}%</div></div>
    </div>
  </div>
  <div class="section">
    ...
    <table class="info-table">
      ...
      <tr><td>Video Duration</td><td>{duration_sec:.1f} seconds</td></tr>
      <tr><td>Confidence Threshold</td><td>{conf_threshold}</td></tr>
      <tr><td>IoU Threshold</td><td>{IOU_THRESH}</td></tr>
      <tr><td>AI Model</td><td>YOLOv11s (Ultralytics)</td></tr>
      <tr><td>Model Accuracy</td><td>mAP@50: 96.9% | mAP@50-95: 94.1%</td></tr>
      <tr><td>Tracking Algorithm</td><td>BoT-SORT</td></tr>
  ...
  <div class="report-footer">
    <div class="left"><b>VialVision AI</b> — Pharmaceutical Quality Control System<br>Ramaiah Institute of Technology, Bengaluru<br>M.Tech Robotics & AI | Developed by Geeta Siddramayya Math</div>
    <div class="right">Report ID: {report_id}<br>Generated: {date_str} {time_str}<br>YOLOv11s | 96.9% mAP50</div>
  </div>
</div>
</body>
</html>"""
...
def generate_csv_report(vid_counts, video_name, fps, total_frames, W, H, conf_threshold, duration_sec):
    ...
    writer.writerow(["Unsealed",       unsealed, f"{unsealed/max(total,1)*100:.1f}%"])
    writer.writerow(["Damaged",        damaged,  f"{damaged/max(total,1)*100:.1f}%"])
    ...
    writer.writerow(["IoU",          IOU_THRESH])
...
with st.sidebar:
    ...
    <span style="color:#00d4ff">Conf (Image):</span> 0.70<br>
    <span style="color:#00d4ff">Conf (Video):</span> 0.50<br>
    <span style="color:#00d4ff">IoU:</span> 0.30
    </div>
    """, unsafe_allow_html=True)
    st.markdown('<hr class="divider">', unsafe_allow_html=True)
...
if err:
    st.markdown(f'<div class="status-err">⚠ Model Error: {err}</div>', unsafe_allow_html=True)
    st.stop()
else:
    st.markdown('<div class="status-ok">✓ Model loaded successfully — Ready for inference</div>',
                unsafe_allow_html=True)
...
# conf=0.7, iou=0.30 to suppress duplicate boxes
# ══════════════════════════════════════════════
with tab1:
    st.markdown('<br>', unsafe_allow_html=True)
    ...
    if uploaded_imgs:
        for uploaded_file in uploaded_imgs:
            st.markdown(f'<div class="panel-title">🔍 {uploaded_file.name}</div>',
                        unsafe_allow_html=True)
            ...
            with st.spinner("Running inference..."):
                ...
                # conf=0.7 and iou=0.30 suppress overlapping/wrong boxes
                results = model.predict(img_bgr, conf=IMAGE_CONF, iou=IOU_THRESH, verbose=False)
            ...
            with col2:
                ...
                st.markdown('<div class="panel-title" style="margin-top:1rem">DETECTIONS</div>',
                            unsafe_allow_html=True)
                if len(results[0].boxes) == 0:
                    st.markdown('<div class="status-warn">No objects detected</div>',
                                unsafe_allow_html=True)
                else:
                    color_map = {'Vial':'#00d4ff','Sealed':'#00ff88',
                                 'Unsealed':'#ff8c00','Damaged':'#ff3366'}
                    conf_html = ""
                    for box in results[0].boxes:
                        cls  = int(box.cls[0])
                        conf = float(box.conf[0])
                        name = model.model.names.get(cls, 'Unknown')
                        conf_html += conf_bar_html(name, conf, color_map.get(name,'#fff'))
                    st.markdown(conf_html, unsafe_allow_html=True)
                    tags_html = ""
                    for name, cnt in counts.items():
                        tags_html += f'<span class="det-tag {name.lower()}">{name} × {cnt}</span>'
                    st.markdown(tags_html, unsafe_allow_html=True)
            st.markdown('<hr class="divider">', unsafe_allow_html=True)

# ══════════════════════════════════════════════
# TAB 2 — VIDEO DETECTION
# conf=0.50, iou=0.30, BoT-SORT tracking + counting
# ══════════════════════════════════════════════
with tab2:
    st.markdown('<br>', unsafe_allow_html=True)
    ...
    if uploaded_video:
        ...
        if st.button("▶  Process Video", use_container_width=True):
            ...
            while cap.isOpened():
                ...
                if frame_count % process_every == 0:
                    # conf=0.50, iou=0.30 suppress overlapping boxes
                ...
                if cached_results is not None:
                    ...
                    if enable_counting:
                        ...
                        cv2.putText(frame, f"Total:    {total}",             (W-210,35),  fs,0.5,(0,255,255),1,cv2.LINE_AA)
                        cv2.putText(frame, f"Sealed:   {vid_counts['Sealed']}",(W-210,62),fs,0.5,(0,255,136),1,cv2.LINE_AA)
                        cv2.putText(frame, f"Unsealed: {vid_counts['Unsealed']}",(W-210,89),fs,0.5,(255,140,0),1,cv2.LINE_AA)
                        cv2.putText(frame, f"Damaged:  {vid_counts['Damaged']}",(W-210,116),fs,0.5,(0,0,255),1,cv2.LINE_AA)
            ...
            st.session_state['html_report'] = generate_html_report(
                vid_counts, uploaded_video.name, fps, total_frames, W, H, VIDEO_CONF, duration_sec)
        ...
        if st.session_state.get('processing_done', False):
            st.markdown('<div class="status-ok">✓ Video processing complete — Download your files below</div>',
                        unsafe_allow_html=True)
            st.markdown('<br>', unsafe_allow_html=True)
...
with tab3:
    ...
    <div class="panel">
        ...
        <span style="color:#00ff88">• Camera index 0 = Built-in , 1 = Logitech external</span><br>
        <span style="color:#ffe066">• Live tracking + counting with BoT-SORT</span><br>
        <span style="color:#c8d8e8">• Press <b style="color:#ff3366">Stop Camera</b> to end session</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_cam1:
        cam_index    = st.selectbox("Camera Index", [0, 1, 2], index=0,
                                     help="0=Logitech external, 1=built-in")
    with col_cam2:
        cam_line_pos = st.slider("Counting Line Position (%)", 20, 80, 50, key="cam_line")

    col_btn1, col_btn2 = st.columns(2)
    start_cam = col_btn1.button("▶  Start Camera", use_container_width=True)
    stop_cam  = col_btn2.button("⏹  Stop Camera",  use_container_width=True)

    if start_cam:
        st.session_state['cam_running'] = True
        st.session_state['cam_counts']  = defaultdict(int)
        st.session_state['cam_last_x']  = {}
        st.session_state['cam_counted'] = set()
        st.session_state['cam_report_ready'] = False
    if stop_cam:
    ...
    if st.session_state.get('cam_running', False):
        ...
        if not cap.isOpened():
            st.markdown('<div class="status-err">⚠ Cannot access camera. Try index 0 or 1.</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<div class="status-ok">✓ Camera active — Live tracking & counting running</div>',
                        unsafe_allow_html=True)

            CAM_CLASS_MAP = {0: 'Damaged', 1: 'Vial', 2: 'Sealed', 3: 'Unsealed'}
            cam_W         = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))  or 1280
            cam_H         = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or 720
            cam_line_x    = int(cam_line_pos / 100 * cam_W)
            last_x        = st.session_state['cam_last_x']
            counted_ids   = st.session_state['cam_counted']
            cam_counts    = st.session_state['cam_counts']
            cam_ph        = st.empty()
            metrics_ph    = st.empty()
            frame_cnt     = 0
            ...
            while st.session_state.get('cam_running', False):
                ...
                if frame_cnt % 2 == 0:
                    ...
                    cv2.line(frame, (cam_line_x, 0), (cam_line_x, cam_H), (0, 0, 255), 2)
                    ...
                    if (results[0].boxes is not None and
                        ...
                        for box, cls, conf, tid in zip(boxes, clss, confs, ids):
                            x1, y1, x2, y2 = map(int, box)
                            cx    = (x1 + x2) // 2
                            name  = CAM_CLASS_MAP.get(cls, 'Vial')
                            color = CLASS_COLORS.get(name, (255, 255, 255))
                            ...
                            if name == 'Vial':
                                if tid in last_x and tid not in counted_ids:
                                    prev_cx = last_x[tid]
                                    crossed = (
                                        (prev_cx >= cam_line_x and cx < cam_line_x) or
                                        (prev_cx <= cam_line_x and cx > cam_line_x)
                                    )
                                    if crossed:
                                        best_state = 'Sealed'
                                        min_dist   = 100
                            ...
                            if name == 'Vial':
                                ...
                                ly = min(y2 + th + 10, cam_H - 5)
                            cv2.rectangle(frame, (x1, ly-th-6), (x1+tw+8, ly+2), color, -1)
                            cv2.putText(frame, label_txt, (x1+4, ly-2),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 0), 1, cv2.LINE_AA)

                    total   = cam_counts['Sealed'] + cam_counts['Unsealed'] + cam_counts['Damaged']
                    overlay = frame.copy()
                    cv2.rectangle(overlay, (cam_W-220, 10), (cam_W-10, 140), (0, 0, 0), -1)
                    frame   = cv2.addWeighted(overlay, 0.75, frame, 0.25, 0)
                    fs      = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(frame, f"Total:    {total}",               (cam_W-210, 35),  fs, 0.5, (0,255,255), 1)
                    cv2.putText(frame, f"Sealed:   {cam_counts['Sealed']}", (cam_W-210, 62),  fs, 0.5, (0,255,136), 1)
                    cv2.putText(frame, f"Unsealed: {cam_counts['Unsealed']}",(cam_W-210, 89),  fs, 0.5, (255,140,0), 1)
                    cv2.putText(frame, f"Damaged:  {cam_counts['Damaged']}", (cam_W-210, 116), fs, 0.5, (0,0,255),   1)

                    cam_ph.image(
                        cv2.cvtColor(frame, cv2.COLOR_BGR2RGB),
                        caption=f"Live Detection | Counted: {total}",
                        use_container_width=True
                    )
                    live_counts = {
                        'Vial':     total,
                        'Sealed':   cam_counts['Sealed'],
                        'Unsealed': cam_counts['Unsealed'],
                        'Damaged':  cam_counts['Damaged'],
                    }
                    metrics_ph.markdown(metric_cards_html(live_counts), unsafe_allow_html=True)

            cap.release()  # ← only once
            st.session_state['cam_report_ready'] = True

    else:
        st.markdown("""
        <div style="text-align:center;padding:3rem;border:1px dashed #1e3a5f;border-radius:10px;">
            <div style="font-size:3rem;">📷</div>
            <div style="color:#c8d8e8;font-family:'Share Tech Mono',monospace;font-size:0.85rem;margin-top:1rem;">
                CAMERA INACTIVE<br>Click "Start Camera" to begin
            </div>
        </div>
        """, unsafe_allow_html=True)

    # ── Report OUTSIDE if/else — shows after stopping ──────────────
    # Triggered when camera is stopped AND counts exist
    if (st.session_state.get('cam_report_ready', False) and
            not st.session_state.get('cam_running', False) and
            sum(st.session_state.get('cam_counts', {}).values()) > 0):

        cam_counts_final = dict(st.session_state['cam_counts'])
        cam_counts_final['Vial'] = (
            cam_counts_final.get('Sealed', 0) +
            cam_counts_final.get('Unsealed', 0) +
            cam_counts_final.get('Damaged', 0)
        )

        st.markdown('<div class="status-ok">✓ Session complete — Download your report below</div>',
                    unsafe_allow_html=True)
        st.markdown('<br>', unsafe_allow_html=True)
        st.markdown(metric_cards_html(cam_counts_final), unsafe_allow_html=True)

        html_rep = generate_html_report(
            cam_counts_final, "Live Camera Session",
            30, 0, 1280, 720, CAMERA_CONF, 0
        )
        csv_rep = generate_csv_report(
            cam_counts_final, "Live Camera Session",
            30, 0, 1280, 720, CAMERA_CONF, 0
        )
        ...
        with rc1:
            st.download_button(
                ...
                file_name="live_camera_report.html",
                mime="text/html",
                use_container_width=True,
                key="cam_html"
            )
        with rc2:
            st.download_button(
                "📊  Download CSV Report",
                csv_rep.encode('utf-8'),
                file_name="live_camera_report.csv",
                mime="text/csv",
                use_container_width=True,
                key="cam_csv"
            )
