...
# ══════════════════════════════════════════════
# TAB 3 — LIVE CAMERA with Tracking & Counting
# Exact same logic as Tab2
# ══════════════════════════════════════════════
with tab3:
    st.markdown('<br>', unsafe_allow_html=True)
    st.markdown("""
    <div class="panel">
        ...
        <span style="color:#ffe066">• Live tracking + counting — same as video tab</span><br>
    ...
    with col_cam1:
        cam_index = st.selectbox("Camera Index", [0, 1, 2],
                                  index=0, help="0=Logitech external, 1=built-in")
    with col_cam2:
    ...
    if st.session_state.get('cam_running', False):
        ...
        if not cap.isOpened():
            ...
            st.markdown(
                '<div class="status-ok">✓ Camera active — Live tracking & counting running</div>',
                unsafe_allow_html=True
            )

            # ── Same class config as Tab2 ────────────────────
            ...
            # ── Tracking state (persists while camera runs) ──
            ...
            while st.session_state.get('cam_running', False):
                ...
                # ── Run BoT-SORT tracker every 2 frames ──────
                if frame_cnt % 2 == 0:
                    results = model.track(
                        frame, persist=True,
                        tracker="botsort.yaml",
                    ...
                    # ── Draw counting line ────────────────────
                    cv2.line(frame, (cam_line_x, 0), (cam_line_x, cam_H),
                             (255, 255, 255), 1)
                    cv2.line(frame, (cam_line_x, 0), (cam_line_x, cam_H),
                             (0, 0, 255), 2)
                    ...
                    if (results[0].boxes is not None and
                        ...
                        for box, cls, conf, tid in zip(boxes, clss, confs, ids):
                            ...
                            color = CAM_COLORS.get(name, (255, 255, 255))

                            # ── Counting logic (exact same as Tab2) ──
                            if name == 'Vial':
                                if tid in last_x and tid not in counted_ids:
                                    prev_cx = last_x[tid]
                                    crossed_rtl = (prev_cx >= cam_line_x and cx < cam_line_x)
                                    crossed_ltr = (prev_cx <= cam_line_x and cx > cam_line_x)
                                    if crossed_rtl or crossed_ltr:
                                        best_state = 'Sealed'
                                        min_dist   = 100
                                        for d_box, d_name in frame_defects:
                                            d_cx = (d_box[0] + d_box[2]) // 2
                            ...
                            # ── Draw label with ID (same as Tab2) ──
                            cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                            label_txt = f"{name} {conf:.2f} | ID {tid}"
                            ...
                            cv2.rectangle(frame,
                                          ...
                                          color, -1)
                            cv2.putText(frame, label_txt, (x1 + 4, y1 - 6),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.45,
                                        (0, 0, 0), 1, cv2.LINE_AA)

                    # ── Dashboard overlay (same as Tab2) ─────
                    total = cam_counts['Sealed'] + cam_counts['Unsealed'] + cam_counts['Damaged']
                    overlay = frame.copy()
                    cv2.rectangle(overlay, (cam_W - 220, 10), (cam_W - 10, 140),
                                  (0, 0, 0), -1)
                    frame = cv2.addWeighted(overlay, 0.75, frame, 0.25, 0)
                    f_style = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(frame, f"Total:    {total}",
                                (cam_W-210, 35),  f_style, 0.55, (0,255,255), 1)
                    cv2.putText(frame, f"Sealed:   {cam_counts['Sealed']}",
                                (cam_W-210, 60),  f_style, 0.5,  (0,255,136), 1)
                    cv2.putText(frame, f"Unsealed: {cam_counts['Unsealed']}",
                                (cam_W-210, 85),  f_style, 0.5,  (255,140,0), 1)
                    cv2.putText(frame, f"Damaged:  {cam_counts['Damaged']}",
                                (cam_W-210, 110), f_style, 0.5,  (0,0,255),   1)
